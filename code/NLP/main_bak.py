...
from dataloader import map_id_rel
...
def get_train_args():
    ...
    parser.add_argument('--num_workers', type=int,
                        default=2, help='dataloader使用的线程数量')
    parser.add_argument('--num_labels', type=int,
                        default=labels_num, help='分类类数')
...
def eval(net, dataset, batch_size):
    ...
    with torch.no_grad():
        ...
        print("Eval Result: right", correct.cpu().numpy().tolist(),
              "total", total, "Acc:", acc)
        return acc
...
def train(net, dataset, num_epochs, learning_rate,  batch_size):
    ...
    optimizer = AdamW(net.parameters(), lr=2e-4)
    ...
    for epoch in range(num_epochs):
        ...
        print("epoch ", str(epoch), " loss: ", loss.mean().numpy().tolist(), "right", correct.cpu(
        ).numpy().tolist(), "total", total, "Acc:", correct.cpu().numpy().tolist()/total)
        acc = eval(net, dev_dataset, 1)
        if acc > pre:
            pre = acc
            print("TOP ACC: ", acc, " Save Model! ")
...
eval(model, dev_dataset, 1)
train(model, train_dataset, 20, 0.02, 16)
eval(model, dev_dataset, 1)
